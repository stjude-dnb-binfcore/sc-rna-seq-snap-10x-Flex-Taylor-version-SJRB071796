...
1. Pull the singularity container from the `sc-rna-seq-snap-10x-Flex` root_dir
...
Then user may navigate to their module of interest, `./sc-rna-seq-snap-10x-Flex/analyses/<module_of_interest>`. For example:

```
cd ./sc-rna-seq-snap-10x-Flex/analyses/upstream-analysis
...
1. Pull the Docker Container from the `sc-rna-seq-snap-10x-Flex` root_dir:
...
```
docker run --platform linux/amd64 --name review -d -e PASSWORD=ANYTHING -p 8787:8787 -v $PWD:/home/rstudio/sc-rna-seq-snap-10x-Flex docker://achroni/rstudio_4.4.0_seurat_4.4.0:latest
```
